...
We can then proceed to load the dataset, and prepare it for a competing risks analysis using `msprep()`. The steps are described in more detail in the [main vignette](https://cran.r-project.org/web/packages/mstate/vignettes/Tutorial.pdf).
...
These confidence intervals can be ommited using `conf.type = "none"`. 
...
Any plots made with *mstate* using the `use.ggplot = TRUE` will return a ggplot objects, which can be saved to a desired format by using `ggplot2::ggsave()`. 
