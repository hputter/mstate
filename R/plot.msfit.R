    ...
    legend,legend.pos,bty="o",...)
{
    ...
    type <- match.arg(type)
    if (missing(legend)) legend <- as.character(1:K) # TODO: use names of tmat and put -> in between
    if (type=="single") {
        ...
        for (k in 1:K) {
            if (missing(ylim)) plot(msft,msfp[,k],type="s",xlab=xlab,ylab=ylab,col=cols[k],lwd=lwd,...)
            else plot(msft,msfp[,k],type="s",ylim=ylim,xlab=xlab,ylab=ylab,col=cols[k],lwd=lwd,...)
            title(main=paste("Transition",k))
        }
    }
    return(invisible())
}
