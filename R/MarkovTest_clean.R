MarkovTest <- function(data, id, formula = NULL, transition, grid, 
  ...
  MTres <- list(orig_stat = orig_stat, orig_ch_stat = orig_ch_stat, p_stat_wb = p_stat_wb, 
                ...
                qualset = qualset, coxfit = progfit)
...
# Create a function that implements the optimal weighting for the
# chi-squared trace
optimal_weights_multiple <- function(data, grid, from, to, min_time = 0)
{
  numbers <- sapply(grid, function(x)
    table(factor(data$from)[(data$entry <= x & data$exit > x)]))
  subevent <- sapply(grid, function(x)
    sum(data$from == from & data$to == to & data$exit > x))
...
optimal_weights_matrix <- function(data, grid, from, to, min_time = 0, 
                                   other_weights = NULL)
{
  numbers <- sapply(grid, function(x)
    table(factor(data$from)[(data$entry <= x & data$exit > x)]))
